  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24 text-center bg-black">
      ...
      <div className="relative flex items-center justify-center mb-8">
        {isRecording && !error && (
          <>
            <div className="absolute w-32 h-32 rounded-full animate-ping" style={{ backgroundColor: '#4A52EB', opacity: 0.3, animationDuration: '2s' }} />
            <div className="absolute w-40 h-40 rounded-full animate-ping" style={{ backgroundColor: '#4A52EB', opacity: 0.2, animationDuration: '2.5s', animationDelay: '0.3s' }} />
            <div className="absolute w-48 h-48 rounded-full animate-ping" style={{ backgroundColor: '#4A52EB', opacity: 0.1, animationDuration: '3s', animationDelay: '0.6s' }} />
          </>
        )}
      ...
      {result && (
        <div className="mt-8 p-6 rounded-lg text-left w-full max-w-md" style={{ backgroundColor: '#1F1F1F' }}>
          <h2 className="text-2xl font-bold mb-2" style={{ color: '#D1F577' }}>{result.title}</h2>
          <p className="text-lg mb-1" style={{ color: '#F1F1F3' }}>
            by {result.artists.map((a) => a.name).join(', ')}
          </p>
      ...
      {error && (
        <p className="mt-4 text-lg font-medium" style={{ color: '#EF4444' }}>{error}</p>
      )}
    </main>
  );
}
